...
import { API_BORROW_BOOK } from '../../constant/urls'
import { getDateTime } from '../../util/date'

class BorrowBook extends Component {
  async doBorrow(values) {
    const r = await axios.post(API_BORROW_BOOK, values);
    if (r && r.status === 200) {
      if (r.data.code) {
  ...
  handleSubmit = () => {
    this.props.form.validateFields((err, values) => {
      if (err) {
        ...
        values['borrow_time'] = getDateTime();
        this.doBorrow(values);
...
export default Form.create()(BorrowBook);